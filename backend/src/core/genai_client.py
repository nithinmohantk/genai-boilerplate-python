...
class GenAIClient:
    ...
    async def get_available_models(
        ...
        try:
            ...
            if provider == "all":
                models_dict = (
                    get_production_models() if not include_experimental else ALL_MODELS
                )
            else:
                ...
                models_list.append(
                    {
                        ...
                        "capabilities": [
                            cap.value for cap in model_info.get("capabilities", [])
                        ],
                        "supports_streaming": model_info.get(
    ...
    def get_models_by_capability(self, capability: str) -> list[dict[str, Any]]:
        ...
        from config.ai_models import ModelCapability, get_models_with_capability
