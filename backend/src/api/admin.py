...
async def get_all_tenants(
    ...
    try:
        query = (
            select(Tenant).order_by(desc(Tenant.created_at)).limit(limit).offset(offset)
        )
...
async def create_tenant_admin(
    ...
    try:
        ...
        logger.info(
            f"Tenant created: {tenant.id} by super admin {current_user.user_id}"
        )
...
async def update_tenant_admin(
    ...
    try:
        ...
        logger.info(
            f"Tenant updated: {tenant_id} by super admin {current_user.user_id}"
        )
...
async def create_user_admin(
    ...
    try:
        ...
        existing_user = await auth_service.get_user_by_email(
            user_data.email, current_user.tenant_id
        )
        if existing_user:
            raise HTTPException(
...
async def update_user_admin(
    ...
    try:
        ...
        updated_user = await auth_service.update_user_profile(
            user_id=user_id, updates=user_updates.dict(exclude_unset=True)
        )
...
async def create_api_key(
    ...
    try:
        ...
        existing_key_query = select(TenantApiKey).where(
...
async def delete_api_key(
    ...
    try:
        ...
        query = select(TenantApiKey).where(
            TenantApiKey.id == key_id, TenantApiKey.tenant_id == current_user.tenant_id
        )
        result = await db.execute(query)
