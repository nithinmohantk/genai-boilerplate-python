...
async def get_themes(
    ...
    themes = await theme_service.get_themes(
        category=category, include_system=include_system
    )
    return themes
...
async def create_theme(
    ...
    theme = await theme_service.create_theme(
        theme_data=theme_data, created_by=current_user.id
    )
    return theme
...
async def update_theme(
    ...
    theme = await theme_service.update_theme(
        theme_id=theme_id, theme_data=theme_data, updated_by=current_user.id
    )
    if not theme:
        raise HTTPException(
...
async def get_theme_css(
    ...
    return {"theme_id": theme_id, "display_mode": display_mode, "css": css}
...
@router.post(
    "/user/settings",
    response_model=UserThemeSettings,
...
async def apply_theme_to_user(
    ...
    settings = await theme_service.apply_theme_to_user(
        user_id=current_user.id, theme_id=theme_id
    )
    if not settings:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def get_current_user_theme_css(
    ...
    if css is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User theme not found"
        )

    return {"user_id": current_user.id, "css": css}
