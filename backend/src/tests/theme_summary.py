...
def analyze_theme_file():
    ...
    theme_file = os.path.join(os.path.dirname(__file__), "..", "services", "theme_init_service.py")
    ...
    if len(theme_names) != len(display_names):
        issues.append(f"Mismatch between theme names ({len(theme_names)}) and display names ({len(display_names)})")
    
    if len(theme_names) != len(descriptions):
        issues.append(f"Mismatch between theme names ({len(theme_names)}) and descriptions ({len(descriptions)})")
    
    if light_count != dark_count:
        issues.append(f"Mismatch between light mode ({light_count}) and dark mode ({dark_count}) definitions")
    
    expected_themes = 12  # Based on our design
    if len(theme_names) < expected_themes:
        issues.append(f"Expected at least {expected_themes} themes, found {len(theme_names)}")
