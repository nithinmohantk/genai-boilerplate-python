...
async def test_theme_definitions():
    ...
    for theme_data in themes_data:
        ...
        required_fields = ['name', 'display_name', 'description', 'category', 'color_scheme']
        for field in required_fields:
            assert field in theme_data, f"Missing required field '{field}' in theme '{theme_data.get('name')}'"
        
        # Check color scheme structure
        color_scheme = theme_data['color_scheme']
        assert 'light' in color_scheme, f"Missing 'light' color scheme in theme '{theme_data['name']}'"
        assert 'dark' in color_scheme, f"Missing 'dark' color scheme in theme '{theme_data['name']}'"
        
        # Check basic colors exist
        basic_colors = ['primary', 'secondary', 'accent', 'background', 'surface', 'text']
        for mode in ['light', 'dark']:
            for color in basic_colors:
                assert color in color_scheme[mode], f"Missing '{color}' in {mode} mode for theme '{theme_data['name']}'"
...
async def test_theme_categories():
    ...
    for theme_data in themes_data:
        ...
        assert category in ThemeCategory, f"Invalid category '{category}' in theme '{theme_data['name']}'"
...
async def test_color_accessibility():
    ...
    if accessibility_issues:
        logger.warning(f"Found {len(accessibility_issues)} potential accessibility issues:")
...
async def test_theme_creation():
    ...
    assert len(created_themes) == expected_count, f"Expected {expected_count} themes, got {len(created_themes)}"
