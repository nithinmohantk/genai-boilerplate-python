...
THEME_SERVICE_PATH = os.path.join(os.path.dirname(__file__), 
                                  "..", "services", "theme_init_service.py")
            ...
            # Inside a theme definition
                ...
                if line.startswith("},"):
                    ...
                    for category in ThemeCategory:
                        theme_str = theme_str.replace(f"ThemeCategory.{category.name}", f'"{category.value}"')
    ...
    for theme in themes:
        ...
        for mode in ["light", "dark"]:
            ...
            if not isinstance(colors, dict):
                logger.error(f"Theme {theme_name} has invalid {mode} color scheme format")
                is_valid = False
                continue
            ...
            for color in basic_colors:
                if color not in colors:
                    logger.error(f"Theme {theme_name} is missing {color} in {mode} mode")
                    is_valid = False
                    
            # Validate color values are hex format
            for color_name, color_value in colors.items():
                if not isinstance(color_value, str) or not color_value.startswith("#"):
                    logger.error(f"Theme {theme_name} has invalid color value for {color_name} in {mode} mode: {color_value}")
                    is_valid = False
...
async def check_theme_categories(themes: List[Dict[str, Any]]) -> bool:
    ...
    missing_categories = set(cat.value for cat in ThemeCategory) - set(category_counts.keys())
