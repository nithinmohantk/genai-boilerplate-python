...
from models.chat import (
    UserMemory, MemoryType, MemoryImportance,
    ...
    async def get_user_memories(
        ...
        try:
            ...
            if tags:
                ...
                conditions.append(
                    UserMemory.tags.op('@>')([tags])  # PostgreSQL JSON contains operator
                )
    ...
    async def search_memories(
        ...
        try:
            ...
            if search_request.memory_types:
                conditions.append(UserMemory.memory_type.in_(search_request.memory_types))
            
            if search_request.importance_levels:
                conditions.append(UserMemory.importance.in_(search_request.importance_levels))
    ...
    async def get_relevant_memories(
        ...
        try:
            ...
            if importance_threshold != MemoryImportance.LOW:
                important_types = [k for k, v in importance_order.items() if v >= min_importance]
        ...
        try:
            ...
            for memory_data in memories_to_create:
                memory = UserMemory(
                    ...
                    memory_type=memory_data["memory_type"],
                    importance=memory_data["importance"],
                    context={"extracted_from": "conversation", "session_id": str(session_id)},
                    embedding=await self._generate_embedding(memory_data["content"])
                )
