...
class DocumentService:
    ...
    async def process_document(self, document_id: UUID) -> bool:
        ...
        try:
            ...
            text_content = await self._extract_text(document.file_path, document.content_type)
            if not text_content:
                return False
            ...
            for i, chunk_text in enumerate(chunks):
                chunk = DocumentChunk(
                    ...
                    content=chunk_text,
                    chunk_index=i,
                    metadata={"page": None, "section": None},  # TODO: Extract from document
            ...
            document.processing_metadata = {
                ...
                "total_tokens": sum(chunk.token_count for chunk in chunk_objects if chunk.token_count),
                "processing_time": datetime.now().isoformat()
            }
            
            await self.db.commit()
            
            logger.info(f"Document processed: {document_id} - {len(chunks)} chunks created")
            return True
