...
from models.chat import (
    AIPersona, UserPersonaSession, PersonaType,
    PersonaCreate, PersonaUpdate, PersonaResponse, PersonaSessionRequest
)
...
class PersonaService:
            ...
            if user_id and tenant_id:
                conditions.append(
                    or_(
                        ...
                        and_(AIPersona.is_public == True, AIPersona.tenant_id == tenant_id)  # Public tenant persona
                    )
                )
    ...
    async def set_session_persona(
        self,
        user_id: UUID,
        persona_request: PersonaSessionRequest
    ) -> Optional[UserPersonaSession]:
        ...
        try:
            ...
            if persona_request.session_id:
                conditions.append(UserPersonaSession.session_id == persona_request.session_id)
            else:
            ...
            if existing_assignment:
                ...
                existing_assignment.custom_prompt_additions = persona_request.custom_prompt_additions
                existing_assignment.temperature_override = persona_request.temperature_override
    ...
    async def get_default_persona(self) -> Optional[AIPersona]:
    ...
    async def generate_persona_prompt(
        ...
        try:
            ...
            if persona and persona.communication_style:
                ...
                if style_parts:
                    prompt_parts.append(f"Communication style - {', '.join(style_parts)}")
            ...
            if session_id:
                ...
                if assignment and assignment.custom_prompt_additions:
                    prompt_parts.append(f"Additional instructions: {assignment.custom_prompt_additions}")
    ...
    async def get_persona_stats(
        ...
        try:
            ...
            popular_personas = [{"name": row.name, "usage_count": row.usage_count} 
            ...
            return {
                ...
                "public_personas": personas_by_type.get(PersonaType.USER, 0)  # Public user personas
            }
            ...
            system_personas = [
                {
                    ...
                    "communication_style": {"tone": "professional", "formality": "balanced"},
                    "temperature": 0.7
                },
                {
                    ...
                    "expertise_areas": ["creative writing", "storytelling", "literature"],
                    "temperature": 0.9
                },
                {
                    ...
                    "communication_style": {"tone": "professional", "formality": "formal"},
                    "expertise_areas": ["programming", "engineering", "technology", "mathematics"],
                    "temperature": 0.3
                },
                {
                    "name": "Casual Friend",
                    ...
                    "personality_traits": ["friendly", "casual", "warm", "approachable"],
