...
from models.theme import (
    Theme, UserSettings, UserThemeHistory, ThemeCategory, DisplayMode,
    ThemeCreate, ThemeUpdate, ThemeResponse, UserSettingsCreate, UserSettingsUpdate,
    ThemeSearchRequest, AccessibilityCheckResponse, FontSize, FontFamily
)
...
class ThemeService:
    ...
    async def get_themes(
        self,
        search_request: Optional[ThemeSearchRequest] = None,
        ...
        try:
            ...
            if search_request:
                ...
                if search_request.supports_dark_mode is not None:
                    conditions.append(Theme.supports_dark_mode == search_request.supports_dark_mode)
            ...
            query = (
                select(Theme)
                .where(and_(*conditions)) if conditions else select(Theme)
            )
            query = (
                query.order_by(
    ...
    async def update_user_settings(
        ...
        try:
            ...
            settings.settings_backup = {
                "active_theme_id": str(settings.active_theme_id) if settings.active_theme_id else None,
    ...
    async def apply_theme_to_user(
        ...
        try:
            ...
            settings_update = UserSettingsUpdate(
                active_theme_id=theme_id,
                display_mode=display_mode or DisplayMode.AUTO
            )
    ...
    async def check_theme_accessibility(self, theme_id: UUID) -> AccessibilityCheckResponse:
        ...
        try:
            ...
            if not wcag_aa_compliant:
                accessibility_issues.append("Background-text contrast ratio below WCAG AA standards")
                recommendations.append("Increase contrast between background and text colors")
            
            if not wcag_aaa_compliant:
                accessibility_issues.append("Background-text contrast ratio below WCAG AAA standards")
            
            # Check for accessibility features
            if not theme.accessibility_features:
                accessibility_issues.append("No accessibility features defined")
                recommendations.append("Add accessibility features like focus indicators and high contrast support")
    ...
    async def get_theme_usage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            popular_themes = [
                {"name": row.display_name, "usage_count": row.usage_count, "category": row.category}
